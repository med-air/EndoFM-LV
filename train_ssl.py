...
from datasets import Kinetics, MaskingGenerator, collate_data_and_cast
...
# from fsdp import reshard_fsdp_model
...
def get_args_parser():
    parser = argparse.ArgumentParser('Endo-FM', add_help=False)
...
def train_svt(args):
    ...
    n_tokens = (224 // args.patch_size) ** 2
    mask_generator = MaskingGenerator(
        input_size=(224 // args.patch_size, 224 // args.patch_size),
        max_num_patches=0.5 * 224 // args.patch_size * 224 // args.patch_size,
    )
    ...
    ibot_loss = iBOTPatchLoss(args.out_dim)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, ibot_loss, koleo_loss, teacher_temp_schedule,
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, ibot_loss, koleo_loss, teacher_temp_schedule, data_loader,
    ...
    for it, (images, _, _, meta) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        # move images, masks to gpu
        for _ in ["collated_global_crops", "collated_local_crops"]:
            images[_] = [im.cuda(non_blocking=True) for im in images[_]]
        for _ in ["collated_masks", "mask_indices_list", "n_masked_patches", "masks_weight"]:
            images[_] = images[_].cuda(non_blocking=True)
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            student_local_cls, student_local_patch = student(images["collated_local_crops"])
            student_global_cls, student_global_patch, student_global_cls_tokens = student(images["collated_global_crops"], return_tokens=True)
            # only the 2 global views pass through the teacher
            teacher_global_cls, teacher_global_patch = teacher(images["collated_global_crops"])
            ...
            _dim = teacher_global_patch.shape[-1]
            n_cls_tokens = teacher_global_cls.shape[0]
            buffer_tensor_teacher_patch = teacher_global_patch.new_zeros(upperbound + n_cls_tokens, _dim)
            buffer_tensor_teacher_patch[:n_cls_tokens].copy_(teacher_global_cls)
            torch.index_select(
                    teacher_global_patch.flatten(0, 1),
                    dim=0,
                    index=mask_indices_list,
                    out=buffer_tensor_teacher_patch[n_cls_tokens: n_cls_tokens + n_masked_patches],
            )
            buffer_tensor_teacher_center = buffer_tensor_teacher_patch[:n_cls_tokens]
            masked_teacher_global_patch = buffer_tensor_teacher_patch[n_cls_tokens: n_cls_tokens + n_masked_patches]
            # print('masked_teacher_global_patch', masked_teacher_global_patch.shape, _dim, n_cls_tokens)

            teacher_dino_softmaxed_centered_list = dino_loss.sinkhorn_knopp_teacher(
                    buffer_tensor_teacher_center, teacher_temp=teacher_temp_schedule[epoch]
                ).view(n_global_crops, -1, *buffer_tensor_teacher_center.shape[1:])

            masked_teacher_global_patch_softmaxed_centered = ibot_loss.sinkhorn_knopp_teacher(
                    masked_teacher_global_patch,
            ...
            buffer_tensor_student_patch = student_global_patch.new_zeros(upperbound, _dim)
            buffer_tensor_student_patch[:n_masked_patches].copy_(
                torch.index_select(student_global_patch.flatten(0, 1), dim=0, index=mask_indices_list)
            )
            masked_student_global_patch = buffer_tensor_student_patch[:n_masked_patches]

            dino_loss_local = dino_loss(
                student_output_list=student_local_cls.chunk(n_local_crops),
                teacher_out_softmaxed_centered_list=teacher_dino_softmaxed_centered_list,
            ) / (n_global_crops_loss_terms + n_local_crops_loss_terms)

            dino_loss_global = dino_loss(
            ...
            ibot_loss_global = ibot_loss.forward_masked(
                    masked_student_global_patch,
                    masked_teacher_global_patch_softmaxed_centered,
                    student_masks_flat=masks,
                    n_masked_patches=n_masked_patches,
                    masks_weight=masks_weight
            ) * loss_scales * ibot_loss_scale / 2.

            # we don't apply koleo loss between cls tokens of a same image
            koleo_loss_global = sum(koleo_loss(p) for p in student_global_cls_tokens.chunk(2))

            loss = dino_loss_local + dino_loss_global + ibot_loss_global + koleo_loss_global * 0.1

            # print(dino_loss_local.item(), dino_loss_global.item(), ibot_loss_global.item(), koleo_loss_global.item(), loss.item())
            # exit(0)
        ...
        metric_logger.update(dino_loss_local=dino_loss_local.item())
        metric_logger.update(dino_loss_global=dino_loss_global.item())
        metric_logger.update(ibot_loss_global=ibot_loss_global.item())
        metric_logger.update(koleo_loss_global=koleo_loss_global.item())
...
class DINOIBOTLoss(nn.Module):
    ...
    def forward(self, images, student_local_cls, student_local_patch, student_global_cls, student_global_patch,
                teacher_global_cls, teacher_global_patch, epoch):
        """
        ...
        _dim = teacher_global_patch.shape[-1]
        n_cls_tokens = teacher_global_cls.shape[0]
        buffer_tensor_teacher = teacher_global_patch.new_zeros(upperbound + n_cls_tokens, _dim)
        buffer_tensor_teacher[:n_cls_tokens].copy_(teacher_global_cls)
        torch.index_select(
            teacher_global_patch.flatten(0, 1),
            dim=0,
            index=mask_indices_list,
        ...
        buffer_tensor_teacher_center = buffer_tensor_teacher[:n_cls_tokens]
        masked_teacher_patch_tokens_after_head = buffer_tensor_teacher[n_cls_tokens: n_cls_tokens + n_masked_patches]

        # teacher centering and sharpening
        temp = self.teacher_temp_schedule[epoch]
        teacher_out = F.softmax((buffer_tensor_teacher_center - self.center) / temp, dim=-1)
