...
def eval_finetune(args):
    ...
    if args.test:
        ...
        test_stats, f1 = validate_network(val_loader, model, linear_classifier, args.n_last_blocks,
                                          args.avgpool_patchtokens, classnum=args.num_labels)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            test_stats, f1 = validate_network(val_loader, model, linear_classifier, args.n_last_blocks, args.avgpool_patchtokens, classnum=args.num_labels)
...
@torch.no_grad()
def validate_network(val_loader, model, linear_classifier, n, avgpool, classnum=2):
    model.eval()
    ...
    confusion_matrix = np.zeros((classnum, classnum))
    for lt, lp in zip(all_target, all_output):
...
if __name__ == '__main__':
    ...
    ### Endo-LV 1min 3gpu 32 frame 3565 video
    # PolypDiag
    # seed 0:
    # seed 1:
    # seed 2:

    ### Endo-LV 1min 3gpu 32 frame
    # PolypDiag
    # seed 0: 95.1
    # seed 1: 96.7
    # seed 2: 95.9
