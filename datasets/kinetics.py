...
class Kinetics(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=10, get_flow=False):
        ...
        # self._path_to_videos = self._path_to_videos[:10]
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries):
            ...
            output = {}
            output["global_crops"] = [frames[0], frames[1]]

            # # global crops for teacher:
            # output["global_crops_teacher"] = [frames[0], frames[1]]

            # local crops:
            local_crops = frames[2:]
            output["local_crops"] = local_crops
            output["offsets"] = ()

            return output, label, index, meta_data
